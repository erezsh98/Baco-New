...
class AvailableCourtSlot(Base):
    ...
    rental_template_id = Column("rental_tamplate_id", BigInteger, ForeignKey("rental_tamplate.id"))
    hour = Column(Integer)
    curdate = Column(Date)
    taken = Column(DateTime, nullable=True)
    is_holiday = Column(String(1), nullable=True)  # Y or null
    order_id = Column(BigInteger, ForeignKey("court_order.id"), nullable=True)
