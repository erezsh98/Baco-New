    ...
    <main className="min-h-screen bg-canvas p-4">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-ink">ימי חג / סגירה{clubName ? ` — ${clubName}` : ""}</h1>
        ...
        <div className="bg-white rounded-xl shadow p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-6 items-end">
            ...
            {form.ids != null && (
              <button onClick={resetForm} className="text-muted text-sm hover:underline px-2 py-2">ביטול</button>
            )}
          </div>
        </div>
          ...
          spans.length === 0 ? (
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {spans.map((s, i) => (
                    <tr key={s.ids.join(",")} className={`${i % 2 === 0 ? "bg-white" : "bg-canvas"} ${isPast(s) ? "text-muted" : ""}`}>
