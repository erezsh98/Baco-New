...
function ResetForm() {
  ...
  if (token) {
    return (
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
        <h1 className="text-2xl font-bold text-green-800 mb-6 text-center">איפוס סיסמה</h1>
  ...
  if (sent) {
    return (
      <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md text-center">
        <h1 className="text-2xl font-bold text-green-800 mb-4">בדוק את האימייל שלך</h1>
        <p className="text-gray-600 mb-6">שלחנו קישור לאיפוס סיסמה לכתובת <strong>{email}</strong></p>
        <Link href="/login" className="text-green-700 hover:underline text-sm">חזור להתחברות</Link>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
      <h1 className="text-2xl font-bold text-green-800 mb-6 text-center">שכחת סיסמה?</h1>
      {error && <p className="text-red-600 text-sm mb-3">{error}</p>}
      ...
      <p className="text-center text-sm text-gray-500 mt-4">
        <Link href="/login" className="text-green-700 hover:underline">חזור לכניסה</Link>
      </p>
    </div>
  );
}
